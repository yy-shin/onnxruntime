...
class FusionLayerNormalizationNCHW(Fusion):
    ...
    def fuse(self, node, input_name_to_nodes: Dict, output_name_to_node: Dict):
        ...
        weight_nhwc = helper.make_tensor(
            weight_input + "_NHWC", TensorProto.FLOAT, weight.shape, weight
        )

        bias_nhwc = helper.make_tensor(
            bias_input + "_NHWC", TensorProto.FLOAT, weight.shape, weight
        )
        ...
        transpose_output = self.create_transpose_node(layernorm_node_name + "_out_nhwc", [0, 3, 1, 2], last_add_node.output[0])
