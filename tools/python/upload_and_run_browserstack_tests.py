...
def upload_apk_parse_json(post_url, apk_path):
    response = requests.post(post_url, files={"file": open(apk_path, "rb")}, auth=(args.id, args.token))
...
while tests_status == "running":
    ...
    test_response = requests.get(
        'https://api-cloud.browserstack.com/app-automate/espresso/v2/builds/{build_id}'.format(build_id = build_response_json["build_id"]),
        auth=(args.id, args.token)
    )
